...
import {
  ArrowRightFromLine, ArrowRightToLine,
...
const STATIC_SECTIONS: Array<{ title: string; items: PaletteItem[] }> = [
  {
    title: 'Data Flow',
    items: [
...
const TRANSFORM_GROUPS = [
  { group: 'shape' as const,   title: 'SHAPE' },
  { group: 'combine' as const, title: 'COMBINE' },
  { group: 'quality' as const, title: 'QUALITY' },
  { group: 'code' as const,    title: 'CODE' },
]
...
const filteredTransformGroups = computed(() => {
  const q = search.value.trim().toLowerCase()
  return TRANSFORM_GROUPS
    .map(g => ({
      ...g,
      items: Object.values(TRANSFORM_REGISTRY)
        .filter(t => t.paletteGroup === g.group)
        .filter(t => !q || t.label.toLowerCase().includes(q) || t.description.toLowerCase().includes(q)),
    }))
    .filter(g => g.items.length > 0)
})

function onDragStart(event: DragEvent, dragType: NodeType, label: string, dragConfig?: Record<string, unknown>) {
  if (!event.dataTransfer) return
  const payload = dragConfig
    ? JSON.stringify({ type: dragType, config: dragConfig, label })
    : dragType
...
<template>
  <aside class="flex w-52 flex-shrink-0 flex-col border-r bg-background">
    ...
    <div class="flex-1 overflow-y-auto py-1">
      <div
        v-if="filteredStatic.length === 0 && filteredTransformGroups.length === 0"
        class="px-3 py-6 text-center text-xs text-muted-foreground"
      >
        No nodes match "{{ search }}"
      </div>

      <template v-for="section in filteredStatic" :key="section.title">
        ...
        <div
          ...
          @dragstart="onDragStart($event, item.dragType, item.label, item.dragConfig)"
        >
          <div class="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-[5px]" :style="{ background: item.accentColor + '18' }">
      ...
      <template v-if="filteredTransformGroups.length > 0">
        <div class="flex items-center gap-2 px-3 pb-1 pt-3">
          <span class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60">Transforms</span>
          <div class="flex-1 border-t border-border/50" />
        </div>

        <template v-for="group in filteredTransformGroups" :key="group.group">
          <div class="px-3 pb-0.5 pt-2">
            <span class="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/40">{{ group.title }}</span>
          </div>
          <div
