...
<template>
  <div class="flex h-full flex-col overflow-auto">
    <div class="flex items-center justify-between border-b px-[18px] py-[11px]">
      <div>
        <h1 class="text-[15px] font-semibold tracking-tight">Triggers & Schedules</h1>
    ...
    <div class="flex-1 overflow-y-auto p-[18px]">
        ...
        <table class="w-full text-sm">
          <thead class="bg-muted/50">
            <tr>
              ...
              <th class="px-3 py-2 text-left text-[10.5px] font-medium text-muted-foreground">Type</th>
              <th class="px-3 py-2 text-left text-[10.5px] font-medium text-muted-foreground">Schedule / URL</th>
              <th class="px-3 py-2 text-left text-[10.5px] font-medium text-muted-foreground">Next run</th>
          ...
          <tbody class="divide-y">
            <tr v-for="t in store.triggers" :key="t.triggerId" class="hover:bg-muted/30">
              <td class="px-3 py-[7px]">
                <RouterLink to="/workflows" class="text-[11.5px] font-medium hover:underline">
                  {{ t.workflowName }}
                </RouterLink>
              </td>
              <td class="px-3 py-[7px]">
      ...
      <div v-if="showCreate" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40"
        ...
        <div class="w-full max-w-md rounded-[7px] border bg-background p-[18px] shadow-lg">
          ...
          <p class="mt-1 text-[11.5px] text-muted-foreground">Choose a type to configure.</p>
          <div class="mt-4 grid grid-cols-2 gap-2">
            <button class="rounded-[7px] border p-[11px] text-left hover:bg-muted" @click="showCreate = false">
